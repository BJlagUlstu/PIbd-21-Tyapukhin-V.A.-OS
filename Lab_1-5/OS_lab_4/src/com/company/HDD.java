...
public class HDD extends JPanel {
    ...
    public void addToHDD(MyTreeNode file) {
        Main.console.append(String.format("Adding a file of size %d" + "\n", file.getSize()));
        tempFileCluster = file.getFirstCluster();
        for (int i = 0; i < file.getSize(); i++) {
            for (int j = 0; j < diskSectors.length; j++) {
                if (diskSectors[j].isEmpty()) {
                    Main.console.append(String.format("Filling cluster of disk %d with cluster of \"%s\" file" + "\n", j, file, file.getFileExtension()));
    ...
    public void paint(Graphics g) {
        ...
        for (int i = 0; i < diskSectors.length; i++) {
            ...
            if (!diskSectors[i].isEmpty()) {
                ...
                if (diskSectors[i].getSelectionType() == 1)
                    g.setColor(Color.GREEN);
                else if (diskSectors[i].getSelectionType() == 2)
                    g.setColor(Color.RED);
                else if (diskSectors[i].getSelectionType() == 3)
